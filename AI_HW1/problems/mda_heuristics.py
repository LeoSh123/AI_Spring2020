...
class MDAMaxAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        if len(all_certain_junctions_in_remaining_ambulance_path) < 2:
            ...
            return max(self.cached_air_distance_calculator.get_air_distance_between_junctions(junction_1, junction_2)
                                             for junction_1 in all_certain_junctions_in_remaining_ambulance_path
                                             for junction_2 in all_certain_junctions_in_remaining_ambulance_path
...
class MDASumAirDistHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        if len(all_certain_junctions_in_remaining_ambulance_path) < 2:
            ...
            currentJunction = state.current_location  # Initiate current Junction
            nextJunction = state.current_location     # Initiate Next Junction
            remainingJunctions = all_certain_junctions_in_remaining_ambulance_path - {currentJunction} # remainig without current
            sum = 0  # sum of distances

            while len(remainingJunctions) > 0:  # while there is a Junction in path
                min = int(2147483646)                # MAX int
                for junction in remainingJunctions:  # iterate over all possible junctions
                    distance = self.cached_air_distance_calculator.get_air_distance_between_junctions(currentJunction, junction)
                    if distance <= min:
                        min = distance
                        nextJunction = junction  # update minimum
                sum = sum + self.cached_air_distance_calculator.get_air_distance_between_junctions(currentJunction, nextJunction)
                remainingJunctions = remainingJunctions - {nextJunction}
...
class MDAMSTAirDistHeuristic(HeuristicFunction):
    ...
    def _calculate_junctions_mst_weight_using_air_distance(self, junctions: List[Junction]) -> float:
        ...
        for junction in junctions:
            graph.add_node(junction.index)

        for junction1 in junctions:
            for junction2 in junctions:
                if junction1.index != junction2.index:
                    distance = self.cached_air_distance_calculator.get_air_distance_between_junctions(junction1, junction2)
                    graph.add_edge(junction1.index, junction2.index, weight=distance)
...
class MDATestsTravelDistToNearestLabHeuristic(HeuristicFunction):
    ...
    def estimate(self, state: GraphProblemState) -> float:
        ...
        labs = set(self.problem.problem_input.laboratories)
        ...
        if currentSite not in labs:
            firstSum = air_dist_to_closest_lab(state.current_location)*state.get_total_nr_tests_taken_and_stored_on_ambulance()
